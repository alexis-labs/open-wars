import { assetPath } from '@deities/art/AssetInfo.tsx';
import { AttackSprite } from '@deities/athena/info/AttackSprite.tsx';

const AttackAcidSprite =
  assetPath('assets/AttackAcid.png');

const AttackAcidCloudSprite =
  assetPath('assets/AttackAcidCloud.png');

const AttackAmphibiousSprite =
  assetPath('assets/AttackAmphibious.png');

const AttackAntiAirSprite =
  assetPath('assets/AttackAntiAir.png');

const AttackArtillerySprite =
  assetPath('assets/AttackArtillery.png');

const AttackBiteSprite =
  assetPath('assets/AttackBite.png');

const AttackBombSprite =
  assetPath('assets/AttackBomb.png');

const AttackCannonSprite =
  assetPath('assets/AttackCannon.png');

const AttackEmptySprite =
  assetPath('assets/AttackEmpty.png');

const AttackExplosionImpactSprite =
  assetPath('assets/AttackExplosionImpact.png');

const AttackFlamethrowerSprite =
  assetPath('assets/AttackFlamethrower.png');

const AttackFlaskSprite =
  assetPath('assets/AttackFlask.png');

const AttackFlaskExplosionSprite =
  assetPath('assets/AttackFlaskExplosion.png');

const AttackHeavyArtillerySprite =
  assetPath('assets/AttackHeavyArtillery.png');

const AttackLightGunSprite =
  assetPath('assets/AttackLightGun.png');

const AttackMGSprite = assetPath('assets/AttackMG.png');

const AttackMinigunSprite =
  assetPath('assets/AttackMinigun.png');

const AttackPistolSprite =
  assetPath('assets/AttackPistol.png');

const AttackPowSprite = assetPath('assets/AttackPow.png');

const AttackPunchSprite =
  assetPath('assets/AttackPunch.png');

const AttackRailgunSprite =
  assetPath('assets/AttackRailgun.png');

const AttackRailgunImpactSprite =
  assetPath('assets/AttackRailgunImpact.png');

const AttackRocketSprite =
  assetPath('assets/AttackRocket.png');

const AttackRocketLauncherSprite =
  assetPath('assets/AttackRocketLauncher.png');

const AttackRocketsSprite =
  assetPath('assets/AttackRockets.png');

const AttackSAMSprite = assetPath('assets/AttackSAM.png');

const AttackSAMImpactSprite =
  assetPath('assets/AttackSAMImpact.png');

const AttackShotgunSprite =
  assetPath('assets/AttackShotgun.png');

const AttackSmokeSprite =
  assetPath('assets/AttackSmoke.png');

const AttackSniperRifleSprite =
  assetPath('assets/AttackSniperRifle.png');

const AttackSparkSprite =
  assetPath('assets/AttackSpark.png');

const AttackSparkMiniSprite =
  assetPath('assets/AttackSparkMini.png');

const AttackTorpedoSprite =
  assetPath('assets/AttackTorpedo.png');

const CraneSprite = assetPath('assets/Crane.png');

const CursorSprite = assetPath('assets/Cursor.png');

const DamageSprite = assetPath('assets/Damage.png');

const DeleteSprite = assetPath('assets/Delete.png');

const ExplosionSprite = assetPath('assets/Explosion.png');

const FireworksSprite = assetPath('assets/Fireworks.png');

const GamepadPlaystationSprite =
  assetPath('assets/GamepadPlaystation.png');

const GamepadSwitchSprite =
  assetPath('assets/GamepadSwitch.png');

const GamepadXboxSprite =
  assetPath('assets/GamepadXbox.png');

const HealSprite = assetPath('assets/Heal.png');

const MessageShadowSprite =
  assetPath('assets/MessageShadow.png');

const NoiseSprite =
  'data:image/png;base64,';

const PoisonSprite = assetPath('assets/Poison.png');

const SabotageSprite = assetPath('assets/Sabotage.png');

const ShieldSprite = assetPath('assets/Shield.png');

const StructuresSprite =
  assetPath('assets/Structures.png');

const TileDecoratorsSprite =
  assetPath('assets/TileDecorators.png');

const UnitIconsSprite = assetPath('assets/UnitIcons.png');

const UpgradeSprite = assetPath('assets/Upgrade.png');

const CrystalsSprite =
  assetPath('assets/render/Crystals.png');

const PortraitSilhouettesSprite =
  assetPath('assets/render/PortraitSilhouettes.png');

const Tiles0Sprite =
  assetPath('assets/render/Tiles0.png');

const Tiles1Sprite =
  assetPath('assets/render/Tiles1.png');

const Tiles2Sprite =
  assetPath('assets/render/Tiles2.png');

const Tiles3Sprite =
  assetPath('assets/render/Tiles3.png');

const Tiles4Sprite =
  assetPath('assets/render/Tiles4.png');

const Tiles5Sprite =
  assetPath('assets/render/Tiles5.png');

const Tiles6Sprite =
  assetPath('assets/render/Tiles6.png');

const UnitsAIUSprite =
  assetPath('assets/shadow/Units-AIU.png');

const UnitsAPUSprite =
  assetPath('assets/shadow/Units-APU.png');

const UnitsAcidBomberSprite =
  assetPath('assets/shadow/Units-AcidBomber.png');

const UnitsAlienSprite =
  assetPath('assets/shadow/Units-Alien.png');

const UnitsAmphibiousSprite =
  assetPath('assets/shadow/Units-Amphibious.png');

const UnitsArtilleryHumveeSprite =
  assetPath('assets/shadow/Units-ArtilleryHumvee.png');

const UnitsBazookaBearSprite =
  assetPath('assets/shadow/Units-BazookaBear.png');

const UnitsBearSprite =
  assetPath('assets/shadow/Units-Bear.png');

const UnitsBomberSprite =
  assetPath('assets/shadow/Units-Bomber.png');

const UnitsBruteSprite =
  assetPath('assets/shadow/Units-Brute.png');

const UnitsCannonSprite =
  assetPath('assets/shadow/Units-Cannon.png');

const UnitsCommanderSprite =
  assetPath('assets/shadow/Units-Commander.png');

const UnitsCustomInfantry2Sprite =
  assetPath('assets/shadow/Units-CustomInfantry-2.png');

const UnitsCustomRocketLauncher2Sprite =
  assetPath('assets/shadow/Units-CustomRocketLauncher-2.png');

const UnitsDinosaurSprite =
  assetPath('assets/shadow/Units-Dinosaur.png');

const UnitsDragonSprite =
  assetPath('assets/shadow/Units-Dragon.png');

const UnitsDroneSprite =
  assetPath('assets/shadow/Units-Drone.png');

const UnitsFighterJetSprite =
  assetPath('assets/shadow/Units-FighterJet.png');

const UnitsFlamethrowerSprite =
  assetPath('assets/shadow/Units-Flamethrower.png');

const UnitsHeavyTankSprite =
  assetPath('assets/shadow/Units-HeavyTank.png');

const UnitsHelicopterSprite =
  assetPath('assets/shadow/Units-Helicopter.png');

const UnitsHumveeSprite =
  assetPath('assets/shadow/Units-Humvee.png');

const UnitsHumveeAvengerSprite =
  assetPath('assets/shadow/Units-HumveeAvenger.png');

const UnitsInfantrySprite =
  assetPath('assets/shadow/Units-Infantry.png');

const UnitsInfernoJetpackSprite =
  assetPath('assets/shadow/Units-InfernoJetpack.png');

const UnitsJeepSprite =
  assetPath('assets/shadow/Units-Jeep.png');

const UnitsJetpackSprite =
  assetPath('assets/shadow/Units-Jetpack.png');

const UnitsMammothSprite =
  assetPath('assets/shadow/Units-Mammoth.png');

const UnitsMedicSprite =
  assetPath('assets/shadow/Units-Medic.png');

const UnitsMobileArtillerySprite =
  assetPath('assets/shadow/Units-MobileArtillery.png');

const UnitsOgreSprite =
  assetPath('assets/shadow/Units-Ogre.png');

const UnitsPioneerSprite =
  assetPath('assets/shadow/Units-Pioneer.png');

const UnitsReconDroneSprite =
  assetPath('assets/shadow/Units-ReconDrone.png');

const UnitsRocketLauncherSprite =
  assetPath('assets/shadow/Units-RocketLauncher.png');

const UnitsSaboteurSprite =
  assetPath('assets/shadow/Units-Saboteur.png');

const UnitsScientistSprite =
  assetPath('assets/shadow/Units-Scientist.png');

const UnitsSeaPatrolSprite =
  assetPath('assets/shadow/Units-SeaPatrol.png');

const UnitsSmallHovercraftSprite =
  assetPath('assets/shadow/Units-SmallHovercraft.png');

const UnitsSniperSprite =
  assetPath('assets/shadow/Units-Sniper.png');

const UnitsSuperAPUSprite =
  assetPath('assets/shadow/Units-SuperAPU.png');

const UnitsSuperTankSprite =
  assetPath('assets/shadow/Units-SuperTank.png');

const UnitsTransportHelicopterSprite =
  assetPath('assets/shadow/Units-TransportHelicopter.png');

const UnitsTruckSprite =
  assetPath('assets/shadow/Units-Truck.png');

const UnitsVampireMedicSprite =
  assetPath('assets/shadow/Units-VampireMedic.png');

const UnitsXFighterSprite =
  assetPath('assets/shadow/Units-XFighter.png');

const UnitsZombieSprite =
  assetPath('assets/shadow/Units-Zombie.png');

export const Crystals = (() => {
  const image = new Image();
  image.src = CrystalsSprite;
  return image;
})();

export const PortraitSilhouettes = (() => {
  const image = new Image();
  image.src = PortraitSilhouettesSprite;
  return image;
})();

export const Tiles0 = (() => {
  const image = new Image();
  image.src = Tiles0Sprite;
  return image;
})();

export const Tiles1 = (() => {
  const image = new Image();
  image.src = Tiles1Sprite;
  return image;
})();

export const Tiles2 = (() => {
  const image = new Image();
  image.src = Tiles2Sprite;
  return image;
})();

export const Tiles3 = (() => {
  const image = new Image();
  image.src = Tiles3Sprite;
  return image;
})();

export const Tiles4 = (() => {
  const image = new Image();
  image.src = Tiles4Sprite;
  return image;
})();

export const Tiles5 = (() => {
  const image = new Image();
  image.src = Tiles5Sprite;
  return image;
})();

export const Tiles6 = (() => {
  const image = new Image();
  image.src = Tiles6Sprite;
  return image;
})();

export const AttackSprites: Record<AttackSprite, string> = {
  Acid: AttackAcidSprite,
  AcidCloud: AttackAcidCloudSprite,
  Amphibious: AttackAmphibiousSprite,
  AntiAir: AttackAntiAirSprite,
  Artillery: AttackArtillerySprite,
  Bite: AttackBiteSprite,
  Bomb: AttackBombSprite,
  Cannon: AttackCannonSprite,
  Empty: AttackEmptySprite,
  ExplosionImpact: AttackExplosionImpactSprite,
  Flamethrower: AttackFlamethrowerSprite,
  Flask: AttackFlaskSprite,
  FlaskExplosion: AttackFlaskExplosionSprite,
  HeavyArtillery: AttackHeavyArtillerySprite,
  LightGun: AttackLightGunSprite,
  MG: AttackMGSprite,
  Minigun: AttackMinigunSprite,
  Pistol: AttackPistolSprite,
  Pow: AttackPowSprite,
  Punch: AttackPunchSprite,
  Railgun: AttackRailgunSprite,
  RailgunImpact: AttackRailgunImpactSprite,
  Rocket: AttackRocketSprite,
  RocketLauncher: AttackRocketLauncherSprite,
  Rockets: AttackRocketsSprite,
  SAM: AttackSAMSprite,
  SAMImpact: AttackSAMImpactSprite,
  Shotgun: AttackShotgunSprite,
  Smoke: AttackSmokeSprite,
  SniperRifle: AttackSniperRifleSprite,
  Spark: AttackSparkSprite,
  SparkMini: AttackSparkMiniSprite,
  Torpedo: AttackTorpedoSprite,
} as const;

export const Sprites = {
  Crane: CraneSprite,
  Cursor: CursorSprite,
  Damage: DamageSprite,
  Delete: DeleteSprite,
  Explosion: ExplosionSprite,
  Fireworks: FireworksSprite,
  GamepadPlaystation: GamepadPlaystationSprite,
  GamepadSwitch: GamepadSwitchSprite,
  GamepadXbox: GamepadXboxSprite,
  Heal: HealSprite,
  MessageShadow: MessageShadowSprite,
  Noise: NoiseSprite,
  Poison: PoisonSprite,
  Sabotage: SabotageSprite,
  Shield: ShieldSprite,
  Structures: StructuresSprite,
  TileDecorators: TileDecoratorsSprite,
  UnitIcons: UnitIconsSprite,
  Upgrade: UpgradeSprite,
} as const;

export const ShadowImages = new Map([
  ['Units-AIU', UnitsAIUSprite],
  ['Units-APU', UnitsAPUSprite],
  ['Units-AcidBomber', UnitsAcidBomberSprite],
  ['Units-Alien', UnitsAlienSprite],
  ['Units-Amphibious', UnitsAmphibiousSprite],
  ['Units-ArtilleryHumvee', UnitsArtilleryHumveeSprite],
  ['Units-BazookaBear', UnitsBazookaBearSprite],
  ['Units-Bear', UnitsBearSprite],
  ['Units-Bomber', UnitsBomberSprite],
  ['Units-Brute', UnitsBruteSprite],
  ['Units-Cannon', UnitsCannonSprite],
  ['Units-Commander', UnitsCommanderSprite],
  ['Units-CustomInfantry-2', UnitsCustomInfantry2Sprite],
  ['Units-CustomRocketLauncher-2', UnitsCustomRocketLauncher2Sprite],
  ['Units-Dinosaur', UnitsDinosaurSprite],
  ['Units-Dragon', UnitsDragonSprite],
  ['Units-Drone', UnitsDroneSprite],
  ['Units-FighterJet', UnitsFighterJetSprite],
  ['Units-Flamethrower', UnitsFlamethrowerSprite],
  ['Units-HeavyTank', UnitsHeavyTankSprite],
  ['Units-Helicopter', UnitsHelicopterSprite],
  ['Units-Humvee', UnitsHumveeSprite],
  ['Units-HumveeAvenger', UnitsHumveeAvengerSprite],
  ['Units-Infantry', UnitsInfantrySprite],
  ['Units-InfernoJetpack', UnitsInfernoJetpackSprite],
  ['Units-Jeep', UnitsJeepSprite],
  ['Units-Jetpack', UnitsJetpackSprite],
  ['Units-Mammoth', UnitsMammothSprite],
  ['Units-Medic', UnitsMedicSprite],
  ['Units-MobileArtillery', UnitsMobileArtillerySprite],
  ['Units-Ogre', UnitsOgreSprite],
  ['Units-Pioneer', UnitsPioneerSprite],
  ['Units-ReconDrone', UnitsReconDroneSprite],
  ['Units-RocketLauncher', UnitsRocketLauncherSprite],
  ['Units-Saboteur', UnitsSaboteurSprite],
  ['Units-Scientist', UnitsScientistSprite],
  ['Units-SeaPatrol', UnitsSeaPatrolSprite],
  ['Units-SmallHovercraft', UnitsSmallHovercraftSprite],
  ['Units-Sniper', UnitsSniperSprite],
  ['Units-SuperAPU', UnitsSuperAPUSprite],
  ['Units-SuperTank', UnitsSuperTankSprite],
  ['Units-TransportHelicopter', UnitsTransportHelicopterSprite],
  ['Units-Truck', UnitsTruckSprite],
  ['Units-VampireMedic', UnitsVampireMedicSprite],
  ['Units-XFighter', UnitsXFighterSprite],
  ['Units-Zombie', UnitsZombieSprite],
]);

export default [
  AttackAcidSprite,
  AttackAcidCloudSprite,
  AttackAmphibiousSprite,
  AttackAntiAirSprite,
  AttackArtillerySprite,
  AttackBiteSprite,
  AttackBombSprite,
  AttackCannonSprite,
  AttackEmptySprite,
  AttackExplosionImpactSprite,
  AttackFlamethrowerSprite,
  AttackFlaskSprite,
  AttackFlaskExplosionSprite,
  AttackHeavyArtillerySprite,
  AttackLightGunSprite,
  AttackMGSprite,
  AttackMinigunSprite,
  AttackPistolSprite,
  AttackPowSprite,
  AttackPunchSprite,
  AttackRailgunSprite,
  AttackRailgunImpactSprite,
  AttackRocketSprite,
  AttackRocketLauncherSprite,
  AttackRocketsSprite,
  AttackSAMSprite,
  AttackSAMImpactSprite,
  AttackShotgunSprite,
  AttackSmokeSprite,
  AttackSniperRifleSprite,
  AttackSparkSprite,
  AttackSparkMiniSprite,
  AttackTorpedoSprite,
  CraneSprite,
  CursorSprite,
  DamageSprite,
  DeleteSprite,
  ExplosionSprite,
  FireworksSprite,
  GamepadPlaystationSprite,
  GamepadSwitchSprite,
  GamepadXboxSprite,
  HealSprite,
  MessageShadowSprite,
  NoiseSprite,
  PoisonSprite,
  SabotageSprite,
  ShieldSprite,
  StructuresSprite,
  TileDecoratorsSprite,
  UnitIconsSprite,
  UpgradeSprite,
  CrystalsSprite,
  PortraitSilhouettesSprite,
  Tiles0Sprite,
  Tiles1Sprite,
  Tiles2Sprite,
  Tiles3Sprite,
  Tiles4Sprite,
  Tiles5Sprite,
  Tiles6Sprite,
  UnitsAIUSprite,
  UnitsAPUSprite,
  UnitsAcidBomberSprite,
  UnitsAlienSprite,
  UnitsAmphibiousSprite,
  UnitsArtilleryHumveeSprite,
  UnitsBazookaBearSprite,
  UnitsBearSprite,
  UnitsBomberSprite,
  UnitsBruteSprite,
  UnitsCannonSprite,
  UnitsCommanderSprite,
  UnitsCustomInfantry2Sprite,
  UnitsCustomRocketLauncher2Sprite,
  UnitsDinosaurSprite,
  UnitsDragonSprite,
  UnitsDroneSprite,
  UnitsFighterJetSprite,
  UnitsFlamethrowerSprite,
  UnitsHeavyTankSprite,
  UnitsHelicopterSprite,
  UnitsHumveeSprite,
  UnitsHumveeAvengerSprite,
  UnitsInfantrySprite,
  UnitsInfernoJetpackSprite,
  UnitsJeepSprite,
  UnitsJetpackSprite,
  UnitsMammothSprite,
  UnitsMedicSprite,
  UnitsMobileArtillerySprite,
  UnitsOgreSprite,
  UnitsPioneerSprite,
  UnitsReconDroneSprite,
  UnitsRocketLauncherSprite,
  UnitsSaboteurSprite,
  UnitsScientistSprite,
  UnitsSeaPatrolSprite,
  UnitsSmallHovercraftSprite,
  UnitsSniperSprite,
  UnitsSuperAPUSprite,
  UnitsSuperTankSprite,
  UnitsTransportHelicopterSprite,
  UnitsTruckSprite,
  UnitsVampireMedicSprite,
  UnitsXFighterSprite,
  UnitsZombieSprite,
];
